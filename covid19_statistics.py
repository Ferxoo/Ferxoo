...
sum_people = sum(n_people) # Compute the total number of people
sum_covid  = sum(n_covid) # Compute the total number of new cases
...
for idx, pop in enumerate(n_people):
    ratio = pop / sum_people * 100 # Calculate the ratio of new cases to each region’s population
...
for idx, cas in enumerate(n_covid):
    ratio = cas / sum_covid * 100 # Calculate the ratio of new cases to each region’s population
    print('| %s | %d | %.1f | %.1f |' % (regions[idx], cas, ratio, norm_covid[idx]))
print()
